...
# Any War Converter
version=v3.2.0
changelog='New online feature for custom CAS option!'
...
menu() {
	option=$(zenity --width=390 --height=205 --list --title "MENU" --text "Selecione a conversão que pretende:\n" --radiolist --column "" --column "Opção" TRUE "SYNLAB (Non Docker)" FALSE "DOCKER (For Fast Autodeploy)" FALSE "RESET (Undo All)" 2>/dev/null);
	if [ "$?" != 0 ]; then
		killall -9 zenity
		exit 0
	fi
	...
	case $option in
		...
		"DOCKER (For Fast Autodeploy)")
			...
			context=$(zenity --width=250 --height=165 --entry --title="Context" --text "Indique o contexto a adicionar:" 2>/dev/null);
			if [ "$?" != 0 ]; then
				killall -9 zenity
				exit 0
			fi
...
# Process custom cas (add or remove):
	if [ -f ./cas.war ]; then
		...
		if [ "$(md5sum resources.zip | awk '{ print $1 }')" = "2e806ea31e4dbf6e88d866763098e599" ]; then
